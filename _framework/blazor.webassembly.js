(function () {
    ...
    (function (DotNet) {
        ...
        class JSObject {
            ...
            resolveInvocationHandler(identifier, callType) {
                var _a;
                const cachedFunction = (_a = this._cachedHandlers.get(identifier)) === null || _a === void 0 ? void 0 : _a[callType];
                if (cachedFunction) {
            ...
            addHandlerToCache(identifier, func, callType) {
        ...
        */
        let JSCallType;
        (function (JSCallType) {
            JSCallType[JSCallType["FunctionCall"] = 1] = "FunctionCall";
            JSCallType[JSCallType["ConstructorCall"] = 2] = "ConstructorCall";
            JSCallType[JSCallType["GetValue"] = 3] = "GetValue";
            JSCallType[JSCallType["SetValue"] = 4] = "SetValue";
        })(JSCallType = DotNet.JSCallType || (DotNet.JSCallType = {}));
        ...
        windowObject._cachedHandlers.set("import", { [JSCallType.FunctionCall]: (url) => {
        ...
        class CallDispatcher {
            ...
            invokeJSFromDotNet(identifier, argsJson, resultType, targetInstanceId, callType) {
            ...
            async beginInvokeJSFromDotNet(asyncHandle, identifier, argsJson, resultType, targetInstanceId, callType) {
                try {
            ...
            processJSCall(targetInstanceId, identifier, callType, argsJson) {
                var _a;
                const args = (_a = parseJsonWithRevivers(this, argsJson)) !== null && _a !== void 0 ? _a : [];
        ...
        function findJSFunction(identifier, targetInstanceId, callType) {
            const targetInstance = cachedJSObjectsById[targetInstanceId];
            if (targetInstance) {
                return targetInstance.resolveInvocationHandler(identifier, callType !== null && callType !== void 0 ? callType : JSCallType.FunctionCall);
            }
         ...
         */
        function findObjectMember(obj, identifier) {
        ...
        DotNet.findObjectMember = findObjectMember;
         ...
         */
        function wrapJSCallAsFunction(parent, memberName, callType, identifier) {
            switch (callType) {
                ...
                case JSCallType.SetValue:
                    ...
                    return (...args) => parent[memberName] = args[0];
            }
        }
        function isReadableProperty(obj, propName) {
        ...
        function isWritableProperty(obj, propName) {
        ...
        function getCaptureIdFromElement(element) {
    ...
    function enableJSRootComponents(rendererId, managerInstance, jsComponentParameters, jsComponentInitializers) {
    ...
    function getRendererAttachedPromise(rendererId) {
    ...
    function addDispatchEventMiddleware(middleware) {
    ...
    class EventInfoStore {
        ...
        addActiveGlobalListener(eventName) {
            // If this event name is an alias, update the global listener for the corresponding browser event
            eventName = getBrowserEventName(eventName);
        ...
        decrementCountByEventName(eventName) {
    ...
    function isMetadataComment(node) {
    ...
    function discoverComponents(root, type) {
        switch (type) {
            case 'webassembly':
    ...
    function discoverWebAssemblyOptions(root) {
    ...
    function discoverWebAssemblyPersistedState(node) {
        return discoverBlazorComment(node, blazorWebAssemblyStateCommentRegularExpression);
    }
    function discoverBlazorComment(node, comment, captureName = 'state') {
        if (node.nodeType === Node.COMMENT_NODE) {
            const content = node.textContent || '';
    ...
    function discoverServerComponents(root) {
        const componentComments = resolveComponentComments(root, 'server');
        return componentComments.sort((a, b) => a.sequence - b.sequence);
    }
    function discoverWebAssemblyComponents(node) {
        const componentComments = resolveComponentComments(node, 'webassembly');
        return componentComments;
    }
    function discoverAutoComponents(node) {
        const componentComments = resolveComponentComments(node, 'auto');
        return componentComments;
    }
        ...
        if (candidateStart.textContent) {
            ...
            if (json) {
                ...
                try {
                    ...
                    // by the end marker.
                    const candidateEnd = getComponentEndComment(componentComment, candidateStart, commentNodeIterator);
                    if (type !== componentComment.type) {
                        return undefined;
                    }
                    switch (componentComment.type) {
                        case 'webassembly':
                            return createWebAssemblyComponentComment(componentComment, candidateStart, candidateEnd);
                        case 'server':
                            return createServerComponentComment(componentComment, candidateStart, candidateEnd);
                        case 'auto':
                            return createAutoComponentComment(componentComment, candidateStart, candidateEnd);
                    }
                }
                catch (error) {
    ...
    function assertNotDirectlyOnDocument(marker) {
    ...
    function getComponentEndComment(payload, start, iterator) {
    ...
    function createServerComponentComment(payload, start, end) {
    ...
    function createWebAssemblyComponentComment(payload, start, end) {
    ...
    function createAutoComponentComment(payload, start, end) {
        validateServerComponentPayload(payload);
        validateWebAssemblyComponentPayload(payload);
        return {
            ...payload,
    ...
    function validateServerComponentPayload(payload) {
    ...
    function validateWebAssemblyComponentPayload(payload) {
    ...
    function validateEndComponentPayload(json, prerenderId) {
        const payload = JSON.parse(json);
    ...
    */
    ...
    function toLogicalRootCommentElement(descriptor) {
        ...
        if (end) {
            ...
            let lastMovedChild = null;
            while (lastMovedChild !== end) {
    ...
    function isMathMLElement(element) {
        const closestElement = getClosestDomElement(element);
    ...
    // itself.
    function* depthFirstNodeTreeTraversal(element) {
    ...
    class BrowserRenderer {
        ...
        detachEventHandlersFromElement(element) {
            for (const childNode of depthFirstNodeTreeTraversal(element)) {
                if (childNode instanceof Element) {
                    this.eventDelegator.removeListenersForElement(childNode);
                }
            }
        }
    ...
    function parseMarkup(markup, isSvg, isMathML) {
        if (isSvg) {
    ...
    // Licensed to the .NET Foundation under one or more agreements.
    // The .NET Foundation licenses this file to you under the MIT license.
    var ScrollResetSchedule;
    (function (ScrollResetSchedule) {
        ScrollResetSchedule[ScrollResetSchedule["None"] = 0] = "None";
        ScrollResetSchedule[ScrollResetSchedule["AfterBatch"] = 1] = "AfterBatch";
        ScrollResetSchedule[ScrollResetSchedule["AfterDocumentUpdate"] = 2] = "AfterDocumentUpdate";
    })(ScrollResetSchedule || (ScrollResetSchedule = {}));
    ...
    function navigateToCore(uri, options, skipLocationChangingCallback = false) {
        ...
        if (options.forceLoad || !isWithinBaseUriSpace(absoluteUri) || pageLoadMechanism === 'serverside-fullpageload') {
            ...
            const unreachable = pageLoadMechanism;
    ...
    function currentPageLoadMechanism() {
        if (hasInteractiveRouter()) {
            ...
            const isBlazorWeb = window['Blazor']._internal.isBlazorWeb;
    ...
    function init(callbackWrapper, elem) {
        ...
        elem.addEventListener('cancel', function () {
    ...
    function receiveDotNetDataStream(dispatcher, streamId, data, bytesRead, errorMessage) {
    ...
    class NullLogger {
        static { this.instance = new NullLogger(); }
     ...
     */
    class BinaryMedia {
        static { this.CACHE_NAME = 'blazor-media-cache'; }
        static { this.cachePromise = undefined; }
        static { this.logger = new ConsoleLogger(LogLevel.Warning); }
        static { this.loadingElements = new Set(); }
        static { this.activeCacheKey = new WeakMap(); }
        static { this.tracked = new WeakMap(); }
        static { this.observersByParent = new WeakMap(); }
        static { this.controllers = new WeakMap(); }
        static initializeParentObserver(parent) {
         ...
         */
        static async setContentAsync(element, streamRef, mimeType, cacheKey, totalBytes, targetAttr) {
            if (!element || !cacheKey) {
        ...
        static async streamAndCreateUrl(element, streamRef, mimeType, cacheKey, totalBytes, targetAttr) {
        ...
        static async readAllChunks(element, stream, controller, totalBytes) {
            const chunks = [];
            let bytesRead = 0;
        ...
        static setUrl(element, url, cacheKey, targetAttr) {
            const tracked = this.tracked.get(element);
            if (tracked) {
                try {
                    URL.revokeObjectURL(tracked.url);
        ...
        static async downloadAsync(element, streamRef, mimeType, totalBytes, fileName) {
        ...
        static async writeStreamToFile(element, stream, writer, totalBytes, controller) {
            let written = 0;
            try {
                for await (const chunk of this.iterateStream(stream, controller?.signal)) {
                    if (controller?.signal.aborted) {
        ...
        static setupEventHandlers(element, cacheKey = null) {
            const clearIfActive = () => {
            ...
            if (element instanceof HTMLVideoElement) {
                const onLoadedData = (_e) => clearIfActive();
                element.addEventListener('loadeddata', onLoadedData, { once: true });
            }
        }
        static async *iterateStream(stream, signal) {
         ...
         */
        static extractBaseMimeType(mimeType) {
            const semicolonIndex = mimeType.indexOf(';');
            return semicolonIndex !== -1 ? mimeType.substring(0, semicolonIndex).trim() : mimeType;
        }
        static async tryMediaSourceVideoStreaming(element, streamRef, mimeType, cacheKey, totalBytes) {
            try {
            ...
            try {
                ...
                let displayStream = originalStream;
                if (cacheKey) {
                    try {
                        const cache = await this.getCache();
                        if (cache) {
    ...
    function attachDebuggerHotkey(config) {
    ...
    // The .NET Foundation licenses this file to you under the MIT license.
    class JSInitializer {
        constructor(singleRuntime = true, logger, afterstartedCallbacks, webRendererId = 0) {
            this.singleRuntime = singleRuntime;
            this.logger = logger;
            this.webRendererId = webRendererId;
        ...
        async importInitializersAsync(initializerFiles, initializerArguments) {
            ...
            async function importAndInvokeInitializer(jsInitializer, asset) {
                ...
                function runMultiRuntimeInitializers(jsInitializer, initializerModule, initializerArguments) {
                    const options = initializerArguments[0];
                    ...
                    if (hasOnlyClassicInitializers && !options.enableClassicInitializers) {
                        ...
                        jsInitializer.logger?.log(LogLevel.Warning, `Initializer '${adjustedPath}' will be ignored because multiple runtimes are available. Use 'before(Web|WebAssembly|Server)Start' and 'after(Web|WebAssembly|Server)Started' instead.`);
                    }
                ...
                function runClassicInitializers(jsInitializer, beforeStart, afterStarted, initializerArguments) {
                    if (afterStarted) {
                ...
                function ensureInitializers(options) {
    ...
    // Licensed to the .NET Foundation under one or more agreements.
    // The .NET Foundation licenses this file to you under the MIT license.
    async function fetchAndInvokeInitializers(options, loadedConfig) {
        if (options.initializers) {
            ...
            /* singleRuntime: */ true, undefined, undefined, WebRendererId.WebAssembly);
    ...
    const monoPlatform = {
        ...
        getObjectFieldsBaseAddress: function getObjectFieldsBaseAddress(referenceTypedObject) {
            return (isMonoRuntime
                ? (referenceTypedObject + 8)
                : (referenceTypedObject + 4));
        },
        readInt16Field: function readHeapInt16(baseAddress, fieldOffset) {
            return runtime.getHeapI16(baseAddress + (fieldOffset || 0));
        },
        readInt32Field: function readHeapInt32(baseAddress, fieldOffset) {
            return runtime.getHeapI32(baseAddress + (fieldOffset || 0));
        },
        readUint64Field: function readHeapUint64(baseAddress, fieldOffset) {
            return runtime.getHeapU52(baseAddress + (fieldOffset || 0));
        },
        readObjectField: function readObjectField(baseAddress, fieldOffset) {
            return runtime.getHeapU32(baseAddress + (fieldOffset || 0));
        },
        readStringField: function readStringField(baseAddress, fieldOffset) {
            const fieldValue = runtime.getHeapU32(baseAddress + (fieldOffset || 0));
            if (fieldValue === 0) {
                return null;
            }
            if (!isMonoRuntime) {
                ...
                const length = runtime.getHeapI16(fieldValue + 4);
    ...
    async function importDotnetJs(startOptions) {
    ...
    function prepareRuntimeConfig(options, onConfigLoadedCallback) {
        const config = {
        ...
        const onConfigLoaded = async (loadedConfig) => {
            if (!loadedConfig.environmentVariables) {
                loadedConfig.environmentVariables = {};
            }
            if (loadedConfig.globalizationMode === "sharded" /* GlobalizationMode.Sharded */) {
    ...
    async function createRuntimeInstance(options, onConfigLoaded) {
    ...
    async function configureRuntimeInstance() {
        if (!runtime) {
        ...
        const exports$1 = await runtime.getAssemblyExports('Microsoft.AspNetCore.Components.WebAssembly');
        Object.assign(Blazor._internal, {
            dotNetExports: {
                ...exports$1.Microsoft.AspNetCore.Components.WebAssembly.Services.DefaultWebAssemblyJSRuntime,
            },
        });
        ...
        dispatcher = DotNet.attachDispatcher({
            beginInvokeDotNetFromJS: (callId, assemblyName, methodIdentifier, dotNetObjectId, argsJson) => {
                ...
                const assemblyNameOrDotNetObjectId = dotNetObjectId
                    ...
                    : assemblyName;
                Blazor._internal.dotNetExports.BeginInvokeDotNet(callId ? callId.toString() : null, assemblyNameOrDotNetObjectId, methodIdentifier, argsJson);
            },
            ...
            invokeDotNetFromJS: (assemblyName, methodIdentifier, dotNetObjectId, argsJson) => {
                assertHeapIsNotLocked();
                return Blazor._internal.dotNetExports.InvokeDotNet(assemblyName ? assemblyName : null, methodIdentifier, dotNetObjectId ?? 0, argsJson);
            },
        });
    }
    ...
    class SharedMemoryRenderBatch {
        ...
        updatedComponents() {
            return platform$1.readStructField(this.batchAddress, 0);
        }
        referenceFrames() {
            return platform$1.readStructField(this.batchAddress, arrayRangeReader.structLength);
        }
        disposedComponentIds() {
            return platform$1.readStructField(this.batchAddress, arrayRangeReader.structLength * 2);
        }
        disposedEventHandlerIds() {
            return platform$1.readStructField(this.batchAddress, arrayRangeReader.structLength * 3);
        }
        ...
        disposedComponentIdsEntry(values, index) {
            ...
            return platform$1.readInt32Field(pointer);
        }
        disposedEventHandlerIdsEntry(values, index) {
            const pointer = arrayValuesEntry(values, index, /* long length */ 8);
            return platform$1.readUint64Field(pointer);
        }
    }
    // Keep in sync with memory layout in ArrayRange.cs
    const arrayRangeReader = {
        structLength: 8,
        values: (arrayRange) => platform$1.readObjectField(arrayRange, 0),
        count: (arrayRange) => platform$1.readInt32Field(arrayRange, 4),
    };
    ...
    const arrayBuilderSegmentReader = {
        ...
        values: (arrayBuilderSegment) => {
            ...
            const builder = platform$1.readObjectField(arrayBuilderSegment, 0);
            const builderFieldsAddress = platform$1.getObjectFieldsBaseAddress(builder);
            return platform$1.readObjectField(builderFieldsAddress, 0);
        },
        offset: (arrayBuilderSegment) => platform$1.readInt32Field(arrayBuilderSegment, 4),
        count: (arrayBuilderSegment) => platform$1.readInt32Field(arrayBuilderSegment, 8),
    };
    // Keep in sync with memory layout in RenderTreeDiff.cs
    const diffReader = {
        structLength: 4 + arrayBuilderSegmentReader.structLength,
        componentId: (diff) => platform$1.readInt32Field(diff, 0),
        edits: (diff) => platform$1.readStructField(diff, 4),
    ...
    const editReader = {
        structLength: 20,
        editType: (edit) => platform$1.readInt32Field(edit, 0),
        siblingIndex: (edit) => platform$1.readInt32Field(edit, 4),
        newTreeIndex: (edit) => platform$1.readInt32Field(edit, 8),
        moveToSiblingIndex: (edit) => platform$1.readInt32Field(edit, 8),
        removedAttributeName: (edit) => platform$1.readStringField(edit, 16),
    };
    // Keep in sync with memory layout in RenderTreeFrame.cs
    const frameReader = {
        structLength: 36,
        frameType: (frame) => platform$1.readInt16Field(frame, 4),
        subtreeLength: (frame) => platform$1.readInt32Field(frame, 8),
        elementReferenceCaptureId: (frame) => platform$1.readStringField(frame, 16),
        componentId: (frame) => platform$1.readInt32Field(frame, 12),
        elementName: (frame) => platform$1.readStringField(frame, 16),
        textContent: (frame) => platform$1.readStringField(frame, 16),
        markupContent: (frame) => platform$1.readStringField(frame, 16),
        attributeName: (frame) => platform$1.readStringField(frame, 16),
        attributeValue: (frame) => platform$1.readStringField(frame, 24),
        attributeEventHandlerId: (frame) => platform$1.readUint64Field(frame, 8),
    };
    function arrayValuesEntry(arrayValues, index, itemSize) {
        return platform$1.getArrayEntryPtr(arrayValues, index, itemSize);
    }
    ...
    async function startCore(components, options, resolve, _) {
        ...
        Blazor._internal.navigationManager.listenForNavigationEvents(WebRendererId.WebAssembly, async (uri, state, intercepted) => {
            await dispatcher.invokeDotNetStaticMethodAsync('Microsoft.AspNetCore.Components.WebAssembly', 'NotifyLocationChanged', uri, state, intercepted);
        }, async (callId, uri, state, intercepted) => {
            const shouldContinueNavigation = await dispatcher.invokeDotNetStaticMethodAsync('Microsoft.AspNetCore.Components.WebAssembly', 'NotifyLocationChangingAsync', uri, state, intercepted);
        ...
        Blazor._internal.endUpdateRootComponents = (batchId) => components.onAfterUpdateRootComponents?.(batchId);
    ...
    function loadWebAssemblyPlatformIfNotStarted(serverOptions) {
    ...
    function invokeJSJson(identifier, targetInstanceId, resultType, argsJson, asyncHandle, callType) {
        if (asyncHandle !== 0) {
    ...
    function receiveWebAssemblyDotNetDataStream(streamId, data, bytesRead, errorMessage) {
    ...
    async function boot(options) {
        ...
        const webAssemblyComponents = discoverComponents(document, 'webassembly');
